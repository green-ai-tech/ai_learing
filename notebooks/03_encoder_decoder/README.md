...
## 📚 学习主题
本模块深入学习**编码器 - 解码器架构 (Encoder-Decoder Architecture)** 及其在深度学习中的应用，从传统的自编码器到现代 Transformer 模型，完整覆盖 NLP 核心算法。
...
### 01_图像编码解码
**学习目标**: 理解图像自编码器的基本原理
- 编码器：将图像压缩为低维表示（Latent Space）
- 解码器：从低维表示还原图像
- 训练过程：最小化重建误差（MSE Loss）
- 应用场景：图像压缩、去噪、特征提取
...
- 在 MNIST 数据集上的应用
- 特征空间的可视化与降维
- 理解瓶颈层（Bottleneck）的作用

### 03_Transformer 文本分类 ⭐ **核心内容**
...
| `predict.py` | 推理脚本，加载已训练模型进行预测 | 训练完成后使用 |

#### 🔧 核心模块：
1. **数据集预处理**: 从 CSV 提取高频字符和标签，序列化保存为 `.pk` 文件
2. **数据集编码**: 文本 → 数字序列（词汇映射），统一序列长度（截断/填充）
3. **词嵌入 (Embedding)**: 加载 Word2Vec 预训练模型，构建嵌入矩阵
4. **Transformer 模型**: 位置编码、多头自注意力机制、Transformer Encoder、分类器
5. **模型训练**: 完整的训练循环，学习率调度，最佳模型自动保存
6. **模型评估**: 分类报告 (Precision, Recall, F1)，混淆矩阵可视化
7. **模型推理**: 单条文本预测，批量预测，Top-K 概率输出

### 04_自注意力机制
**学习目标**: 深入理解自注意力机制的数学原理和代码实现
- 注意力分数计算（Query, Key, Value）
...
# 步骤 3: 运行完整版（实际训练，约需 30-50 轮）
python 03_ai_code.py

# 步骤 4: 训练完成后，使用推理脚本测试新文本
python predict.py
```
...
### 文本分类数据集
- **来源**: CSV 格式的训练集和测试集
- **位置**: `03_Transformer文本分类/data/`
- **字段**:
  - `content`: 文本内容（一段话或一篇文章）
  - `label`: 标签类别（如"体育"、"娱乐"、"科技"等）

### 预训练模型
- **Word2Vec**: `data/sgns.wiki.char.bz2`
- **说明**: 在维基百科上训练的字符级词向量
- **维度**: 300 维
...
| 日期 | 更新内容 |
|------|----------|
| 2026-04-11 | 初始版本，整理文件结构，统一路径配置，编写详细说明 |
